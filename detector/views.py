...
ML_DIR = os.path.join(BASE_DIR, "detector", "ml_assets")
...
def detect_text(request):
    ...
    try:
        ...
        X_ling = np.array(
            [[feats.get(f, 0.0) for f in feature_order]],
            dtype=np.float32
        )
        ...
        # --- Classical models ---
        p_num = cat_model.predict_proba(X_ling)[:, 1]

        p_text = text_lr.predict_proba(X_ling)[:, 1]

        X_stack = np.column_stack([p_num, p_text])

        p_raw = stacker.predict_proba(X_stack)[:, 1]

        p_final = calibrator.predict_proba(
            p_raw.reshape(-1, 1)
        )[:, 1]

        prediction = "ai" if p_final[0] >= THRESHOLD else "human"
